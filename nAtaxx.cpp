// 用AI写了bitboard,决策部分查找了一下之前的论文,使用了比较受好评的negamax算法
// 是minimax的修改版本,在搜索树中,每个节点都存储了一个值,表示当前局面对当前玩家的评估值
// 这个值是通过递归计算的,在每个节点上,我们会尝试所有可能的走法,并计算出对手的最优反应
// 然后我们会选择一个走法,使得当前玩家的评估值最大化,同时对手的评估值最小化
// 这样就可以在搜索树中找到一个最优的走法,使得当前玩家的评估值最大化,同时对手的评估值最小化
// 这个算法的核心思想是,在每个节点上,我们会尝试所有可能的走法,并计算出对手的最优反应
// 然后我们会选择一个走法,使得当前玩家的评估值最大化,同时对手的评估值最小化
// 比之前调出来的效果好很多,后续再往上加参数试试
